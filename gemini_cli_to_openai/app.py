...
应用装配：
- 创建 FastAPI 实例，注册路由。
- startup：导入外部凭据、加载池、确保最小凭据数、启动刷新任务。
"""
import logging
...
def create_app(settings: dict) -> FastAPI:
    ...
    _state.api_client = ApiClient(_state.credential_manager)
    ...
    async def _startup():
        try:
            ...
            min_required = int(settings.get("min_credentials", 1))
            active = _state.credential_manager.get_active_credential_count()
            if active < min_required:
                logging.warning("Active credentials fewer than min_credentials. Starting interactive OAuth flow...")
                _interactive_oauth_acquire(settings, _state)
            # 启动后台刷新
            _state.credential_manager.start_refresh_task()
            logging.info(f"Loaded { _state.credential_manager.get_credential_count() } credentials, { _state.credential_manager.get_active_credential_count() } active")
    ...
    async def root():
        return {
            ...
            "endpoints": {
                "openai": {"chat_completions": "/v1/chat/completions", "models": "/v1/models"},
...
def _interactive_oauth_acquire(settings: dict, _state):
    """交互式 OAuth 获取凭据，直到满足 min_credentials。"""
    target = int(settings.get("min_credentials", 1))
    attempts = 0
    max_attempts = max(3, target * 3)
    while _state.credential_manager.get_active_credential_count() < target and attempts < max_attempts:
        attempts += 1
        logging.info(f"OAuth attempt {attempts}/{max_attempts} - active={_state.credential_manager.get_active_credential_count()} target={target}")
        creds = run_oauth_flow()
        if not creds:
            logging.error("OAuth flow returned no credentials. Prompting user to retry...")
            continue
        # 记录关键信息
        has_rt = bool(getattr(creds, 'refresh_token', None))
        has_token = bool(getattr(creds, 'token', None))
        logging.info(f"OAuth received credentials: has_refresh_token={has_rt}, has_access_token={has_token}")
        # 尝试添加至池并持久化
        ok, reason = _state.credential_manager.add_credentials(creds)
        if not ok:
            logging.error(f"Add credential failed: {reason}. Please try a different account or re-consent.")
            continue
        # Onboard（容错处理）
        try:
            # 找到刚刚添加的最后一条
    ...
    if _state.credential_manager.get_active_credential_count() < target:
        logging.error("OAuth acquisition did not reach min_credentials. Service will run with limited capability.")
