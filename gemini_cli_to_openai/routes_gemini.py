...
from .transformers_openai import build_gemini_payload_from_native
...
async def gemini_proxy(request: Request, full_path: str):
    ...
    try:
        ...
        response = api_client.send_gemini_request(
            auth_key, gemini_payload, is_streaming=is_streaming
        )
        return response
    except Exception as e:
        return Response(
            content=json.dumps(
                {"error": {"message": f"Proxy error: {e}", "code": 500}}
            ),
            status_code=500,
            media_type="application/json",
        )
